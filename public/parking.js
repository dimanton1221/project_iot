...
function updatequeue() {
    for (i = 1; i <= 5; i++) {
        if (i <= queueitems) {
            document.getElementById('queue' + i.toString()).src = 'vehicle.png';
        } else {
            document.getElementById('queue' + i.toString()).src = 'vehicle.png';
        }
    }
...
function carexit(slot) {
    if (!parklock) {
        // Mencoba mendapatkan elemen berdasarkan ID
        var carElement = document.getElementById('car' + slot.toString());

        // Periksa apakah elemen ditemukan
        if (carElement === null) {
            // Jika elemen tidak ditemukan, keluar dari fungsi atau lakukan sesuatu yang sesuai
            console.log("Element not found for slot " + slot);
            return;
        }

        // Melakukan operasi lainnya karena elemen ditemukan
        parklist[slot] = 0;
        ...
        if (slot <= 4)
            carElement.style.animation = 'car-exit-top 2s both';
        else
            carElement.style.animation = 'car-exit-bottom 2s both';
...
// Fungsi untuk mengganti kondisi mobil (masuk/keluar) berdasarkan slot dan kondisi
function changeCarCondition(slot, condition) {
    if (condition === 1) {
        carEnter(slot);
    } else if (condition === 0) {
        carExit(slot);
